...
-- ===== 20260818040000_nutrition_appointments.sql (consulta/agenda/pagamento próprios) =====
...
alter table public.nutritionists add column if not exists pix_profile jsonb;
alter table public.nutritionists add column if not exists commission_percent integer;
alter table public.nutritionists add column if not exists payout_status text not null default 'active';
create table if not exists public.nutrition_appointments (
  ...
  modality text default 'teleconsulta',
...
-- Medicamentos do paciente + adesão diária
