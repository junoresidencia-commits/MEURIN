...
export default function MinhaEquipePage() {
  ...
  async function addExisting(p: Pro) {
    ...
    try {
      ...
      setMsg("Profissional adicionado à sua equipe.");
      await load();
      ...
      <div className="min-w-0 flex-1">
        <div className="mx-auto max-w-3xl px-5 pb-28 pt-8 lg:pb-8">
          ...
          <section className="mt-8">
            ...
            <div className="mt-3 grid gap-2">
              {available.length === 0 && <p className="text-sm text-[var(--text-muted)]">Nenhum profissional disponível.</p>}
              {available.map((p) => (
                <div key={p.id} className="panel flex flex-wrap items-center justify-between gap-2">
                  <div>
                    <p className="font-semibold text-[var(--text)]">{p.name} {p.registry && <span className="text-sm font-normal text-[var(--text-muted)]">· {meta.registry} {p.registry}</span>}</p>
                    <p className="text-xs text-[var(--text-muted)]">{p.status === "pending" ? "Cadastro pendente — ao adicionar, o acesso é liberado" : p.email}</p>
                  </div>
                  <button type="button" className="btn-gold text-sm" onClick={() => addExisting(p)} disabled={saving}>Adicionar</button>
                </div>
              ))}
            </div>
          </section>
