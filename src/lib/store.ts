...
function mapDoctorRow(row: Record<string, unknown>): Doctor {
  return {
    ...
    signatureVisual: (row.signature_visual as Doctor["signatureVisual"]) ?? undefined,
    pixProfile: (row.pix_profile as Doctor["pixProfile"]) ?? undefined,
    stripeConnectReady: Boolean(row.stripe_connect_ready),
    weeklyAvailability: fromJsonArray<WeeklySlot>(row.weekly_availability),
    locations: Array.isArray(row.locations) ? (row.locations as Doctor["locations"]) : [],
    availabilityPeriods: Array.isArray(row.availability_periods)
      ? (row.availability_periods as Doctor["availabilityPeriods"])
      : [],
    ...
    cns: row.cns ? String(row.cns) : undefined,
...
function mapBookingRow(row: Record<string, unknown>) {
  return {
    ...
    modality: row.modality ? (String(row.modality) as Booking["modality"]) : undefined,
    ...
    reminder24Sent: Boolean(row.reminder_24_sent),
    reminder2Sent: Boolean(row.reminder_2_sent),
...
async function writeSupabaseDb(db: Database): Promise<void> {
  ...
  const doctors = db.doctors.map((doctor) => ({
    ...
    signature_visual: doctor.signatureVisual ?? null,
    pix_profile: doctor.pixProfile ?? null,
    stripe_connect_ready: doctor.stripeConnectReady,
    weekly_availability: doctor.weeklyAvailability,
    locations: doctor.locations ?? [],
    availability_periods: doctor.availabilityPeriods ?? [],
  ...
  const bookings = db.bookings.map((booking) => ({
    ...
    reminder_24_sent: booking.reminder24Sent ?? false,
    reminder_2_sent: booking.reminder2Sent ?? false,
