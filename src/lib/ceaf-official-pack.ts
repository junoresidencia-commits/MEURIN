...
export function ceafPackCandidates(file = CEAF_PACOTE.file) {
  return [
...
export async function readCeafOfficialFile(file = CEAF_PACOTE.file): Promise<{ bytes: Buffer; path: string }> {
  const tried: string[] = [];
...
export async function ceafOfficialFileExists(file = CEAF_PACOTE.file) {
  for (const candidate of ceafPackCandidates(file)) {
    try {
